...
RATING_LABELS = {1:"Penalty or unplayable",2:"Trouble",3:"Playable",4:"Good",5:"Perfect"}
SAFE = 4
BAD = 2

def strokes_for(hcp: int, hole_index: int) -> int:
    rating = HOLE_HANDICAP[hole_index]
...
def last(vals): return vals[-1] if vals else None
...
with st.sidebar:
    ...
    if st.button("Reset current hole"):
...
with c1:
    ...
    for i, r in enumerate([1,2,3,4,5]):
        if cols[i].button(f"{r}\n{RATING_LABELS[r]}", use_container_width=True):
            matt_shots.append(r)
...
def role_advice() -> str:
    ...
    if len(matt_shots)+len(mike_shots)==1:
        ...
        if first_rating is None: return "Waiting for the first tee shot rating."
        if first_rating >= SAFE: return f"{first_who} is safe. {other_who} can ATTACK with an aggressive line to create a birdie look."
        if first_rating <= BAD: return f"{first_who} is in trouble. {other_who} must ANCHOR with a fairway finder and play to the center of the green."
    ...
    matt_safe = (m1 or 0) >= SAFE; mike_safe = (k1 or 0) >= SAFE
    if matt_safe and not mike_safe: return "Matt is safe. Mike ATTACKS the pin if the angle allows. Matt avoids short siding."
    if mike_safe and not matt_safe: return "Mike is safe. Matt ATTACKS with freedom. Mike plays to two putt range for par."
    if mike_safe and matt_safe: return "Both are safe. Choose the best birdie look. One goes flag hunting, the other locks in par."
...
st.markdown("### Live Recommendation"); st.write(role_advice()); st.caption(net_targets_text())
st.markdown("---"); st.markdown("Notes: Matt usually receives a stroke on every hole. On HCP 1 he receives two. Day 2 ringer: press harder on holes not covered on Day 1.")
